...
function extractKeywords(text, maxKeywords = 8) {
    ...
    for (const m of text.matchAll(/[#vV]?\d[\d.,:-]+\w*/g))
        tokens.push(m[0]);
    // CJK name+title
    for (const m of text.matchAll(/[\u4e00-\u9fff]{1,4}(?:\u603B|\u7ECF\u7406|\u8001\u5E08|\u90E8\u957F|\u4E3B\u4EFB|\u5148\u751F|\u5973\u58EB)/g))
        tokens.push(m[0]);
    // Dedupe
    const seen = new Set();
    const result = [];
...
// ---------------------------------------------------------------------------
function registerHooks(api, client, config) {
    // -----------------------------------------------------------------------
    ...
    if (config.autoRecall) {
        api.registerHook("before_agent_start", async (context) => {
            ...
            try {
                ...
                const { context: sessionCtx } = await client.init(7, config.recallLimit, config.recallLimit);
                ...
                const results = (recall.results ?? []).filter((r) => r.score === undefined || r.score === null || r.score >= 0.5);
                if (results.length > 0) {
                    parts.push("  <recall>");
                ...
                api.logger.warn("Awareness auto-recall failed, continuing without memory context", err);
                return;
            }
        }, { priority: 10 });
    }
    ...
    if (config.autoCapture) {
        api.registerHook("agent_end", async (context) => {
            ...
            try {
                ...
                const cleanMsg = (content) => content.replace(/<awareness-memory>[\s\S]*?<\/awareness-memory>/g, "").trim();
                ...
                try {
                    ...
                    if (result.events_processed >= 3) {
                        api.logger.info(`Awareness: insight extraction triggered for session ${client.sessionId} (${result.events_processed} events)`);
                    }
                }
                catch (err) {
                ...
                api.logger.warn("Awareness auto-capture failed, conversation not persisted", err);
            }
        }, { priority: 90 });
    }
}
